...
def _has_cross_shard_fusion(load_config) -> bool:
    """True if ``weight_mapping`` contains a ``WeightConverter`` whose
    operations need every matching source tensor live in one ``convert()``
    call.
    ...
    patterns). Both ALWAYS need every tensor matching their source
    pattern(s) live in one call. A converter with a single wildcard
    source pattern (e.g. nemotron_h's ``.experts.*.weight`` →
    ``MergeModulelist``) still fuses multiple expert tensors that can
    live in different shards, so source-count is the wrong proxy --
    operation type is.

    Multi-source converters are also declined regardless of operation:
    multiple distinct source patterns generally imply the converter
    fuses or co-locates them, and at minimum may be order-sensitive.

    Single-source converters with non-hazardous ops (PermuteForRope,
    Transpose, Chunk, etc.) ARE safe under per-shard splits because
    each call independently reproduces the transform on one tensor.
...
def _streaming_load_pretrained_model(
    ...
    for file in checkpoint_files:
        if disable_mmap or _is_on_hf_mount(file):
            # try/finally mirrors the mmap branch below: if _safe_load_bytes
            # or _convert_one_shard raises, the shard dict, page cache, and
            # gc cleanup STILL run so a re-raise during partial mutation
            # doesn't leave a multi-GB shard resident during unwind.
            ...
            try:
                ...
                if shard_state_dict is not None:
                    try:
